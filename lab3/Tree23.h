...
public:
	...
	public:
		...
		bool operator==(Leaf* node); // Проверка равенства однотипных итераторов
		bool operator!=(Leaf* node); // Проверка неравенства однотипных итераторов
		T& operator*();
	private:
		T23<K, T>* tree;
		Leaf* cur;
...
template <class K, class T>
typename T23<K, T>::Leaf* T23<K, T>::rIterator::_FindMin(Internal* t)  //поиск мин. ключа
{
	if (t == NULL) return NULL;
	if ((t->son1->type() == 0))
	{
...
template <class K, class T>
typename T23<K, T>::Leaf* T23<K, T>::rIterator::_FindMax(Internal* t)   //поиск макс. ключа
{
	if (t == NULL)return 0;
	if ((t->son1->type() == 0))
	{
